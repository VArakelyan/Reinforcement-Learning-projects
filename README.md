...
## Project #21 — Mountain Car with Sarsa(λ) & Tile Coding
This project implements the Mountain Car control task using Sarsa(λ) with tile-coded linear function approximation, reproducing the learning behavior shown in Sutton & Barto (Fig. 10.4).
### Overview
- **Environment:** Classic Mountain Car (continuous position & velocity).
- **Actions:** Reverse, zero throttle, forward.
- **Reward:** -1 per step until the goal is reached.
- **Goal:** Learn the optimal “rocking” strategy to build momentum and climb the hill.
