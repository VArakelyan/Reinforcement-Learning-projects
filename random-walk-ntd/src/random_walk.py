    ...
    while True:
        ...
        if update_time >= 0:
            ...
            for t in range(update_time + 1, min(termination_time, update_time + steps_number) + 1):
                estimate_return += pow(discount, t - update_time - 1) * rewards[t]
            ...
            if update_time + steps_number <= termination_time:
                ...
                estimate_return += (pow(discount, steps_number) * state_value_estimates[states[(update_time + steps_number )]])
            ...
            if not state_to_update in terminal_states:
                ...
                state_value_estimates[state_to_update] += (step_size * (estimate_return - state_value_estimates[state_to_update]))
